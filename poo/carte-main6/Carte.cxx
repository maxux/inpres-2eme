...
/* Compare */
bool Carte::IdentNomCaract(Carte &c) {
	return !(strcmp(_name, c.getNomCarte()) == 0);
}
...
Carte & Carte::operator = (Carte &rval) {
	__construct(rval.getNomCarte(), rval.getNomSerie(), rval.getNumero(), rval.getCaractScore().getNomCaract(), rval.getCaractScore().getScore());
	return *this;
}

Carte & Carte::operator + (int value) {
	caract.setScore(caract.getScore() + value);
	return *this;
}
...
int Carte::operator - (Carte &rval) {
	/* if(strcmp(rval.getNomCaract(), _name) != 0)
		return 0; */
	
	return rval.caract.getScore() - rval.caract.getScore();
}
